...
(defun jens/init-fonts ()
  ...
  (let ((my-font "Source Code Pro Semibold 12"))
...
(defun rename-this-file ()
  ...
  (let ((file (buffer-file-name)))
    ...
    (let ((name (buffer-name))
          (new-name (read-file-name "New name: " file)))
      ...
      (message "'%s' successfully renamed to '%s'"
               name (file-name-nondirectory new-name)))))

(defun delete-this-file ()
  "Delete the current buffers file on disk."
  (interactive)
  (let ((file (buffer-file-name))
        (buffer (current-buffer)))
    (when (not (and file (file-exists-p file)))
      (error "Buffer is not visiting a file!"))
    (when (yes-or-no-p (format "really delete '%s'?" file))
      (delete-file file)
...
(use-package notmuch
  ...
  (setq notmuch-wash-citation-lines-prefix 15)
  (setq notmuch-wash-citation-lines-suffix 15)
  (setq notmuch-wash-citation-regexp (rx (1+ (group bol (0+ space) (0+ alpha) ">" (0+ any) "\n"))))
