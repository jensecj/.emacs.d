...
(use-package flycheck :disabled :ensure t)
(use-package git-timemachine :ensure t :functions git-timemachine :defer t)
...
(use-package smex
  ...
  (defun purge-smex-excludes-from-cache ()
  ...
  (advice-add 'counsel-M-x :before #'purge-smex-excludes-from-cache))
...
(use-package jist
  :ensure t
  :functions (jist-region jist-region-public)
  :defer t
