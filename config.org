...
* Use-packages
First, a bit of setup, we want to see what use-package has to say when running.
#+BEGIN_SRC emacs-lisp
(require 'use-package)
...
Load some very useful libraries for working with lists, strings and files.
#+BEGIN_SRC emacs-lisp
(use-package dash :ensure t) ;; lists (-map, -fold, etc.)
(use-package s :ensure t) ;; strings (s-prefix?, etc.)
(use-package f :ensure t) ;; files (f-read, etc.)
#+END_SRC
