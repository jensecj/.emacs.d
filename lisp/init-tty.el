(if (not (window-system))
    (progn
      ...
      (define-key input-decode-map "^[[A" [up]) ;f41
      (define-key input-decode-map "^[[B" [down]) ;f41
      (define-key input-decode-map "^[[C" [right]) ;f41
      (define-key input-decode-map "^[[D" [left]) ;f41
      )
  )
